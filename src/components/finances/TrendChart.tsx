...
export function TrendChart({ data }: { data: DataPoint[] }) {
  ...
  const points = data
    .map((d, i) => `${i * 25},${100 - (d.profit / maxVal) * 100}`)
    .join(" ");
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 h-full flex flex-col">
      <h3 className="text-md font-bold text-gray-800 mb-4">
        Analiza trendów (Zyski w %)
      </h3>
      <div className="grow relative flex items-end px-2 pb-6">
        {/* Linie siatki tła */}
        ...
        <svg
          ...
          aria-label="Trend chart showing profit percentages over months"
          role="img"
        >
          <polyline
            fill="none"
