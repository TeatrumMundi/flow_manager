...
export function VacationsView({
  ...
  useEffect(() => {
    let vacations = initialVacations;

    if (selectedType !== "Wszystkie") {
      vacations = vacations.filter((v) => v.vacationType === selectedType);
    }

    if (selectedStatus !== "Wszystkie") {
      vacations = vacations.filter((v) => v.status === selectedStatus);
    }
  ...
  }, [searchTerm, selectedType, selectedStatus, initialVacations]);
