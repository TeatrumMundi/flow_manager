...
export function EfficiencyChart({ data }: { data: DataPoint[] }) {
    ...
    const getY = (value: number) => {
        return chartHeight - paddingY - (value / maxY) * (chartHeight - 2 * paddingY);
    };
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 h-full flex flex-col">
            <h3 className="text-lg font-bold text-gray-800 mb-6">
                Wydajność <span className="text-sm font-normal text-gray-500">(Wzrost %)</span>
            </h3>
            ...
            <div className="flex-grow relative flex">
                ...
                <div className="flex flex-col justify-between text-xs text-gray-400 font-medium h-full pb-8 pt-2 pr-4">
                    ...
                    <span>8</span>
                    <span>4</span>
                    <span className="opacity-0">0</span> {/* Ukrywamy 0, żeby nie nachodziło na oś X */}
                </div>
                ...
                <div className="flex-grow relative h-full w-full">
                    ...
                    {/* Oś X (Etykiety) */}
                    <div className="absolute bottom-0 left-0 right-0 flex justify-between text-xs text-gray-500 mt-2 px-[10px]"> {/* px dopasowany do paddingu punktów */}
                        {data.map((d) => (
                            <span key={d.label} className="text-center w-8 -ml-4">{d.label}</span>
                        ))}
                    </div>
                </div>
            </div>
