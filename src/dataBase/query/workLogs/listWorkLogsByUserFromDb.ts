...
/**
 * Lists all work logs for a specific user from the database.
 *
 * @param userId - ID of the user whose work logs to retrieve
 * @returns Array of work logs with related task and project data
 *
 * @example
 * ```ts
 * const userWorkLogs = await listWorkLogsByUserFromDb(5);
 * console.log(`User has ${userWorkLogs.length} work log entries`);
 * ```
 */
export async function listWorkLogsByUserFromDb(
  userId: number,
